...
void add_isometric_object_to_order(t_isometric_obj *p_isometric_obj)
{
	int i;
	int ordered_index;
	int height;
...
void main()
{
	...
	while(1)//for(nframes = 256; nframes>=0; nframes--)
	{
		...
		restore_vdisplay_sprite(&(isometric_block_0.sprite));
		restore_vdisplay_sprite(&(isometric_block_1.sprite));
		...
		add_isometric_object_to_order(&isometric_block_0);
		add_isometric_object_to_order(&isometric_block_1);

		for(i=0; i < n_ordered_isometric_objects; i++)
		{
			draw_sprite(&(ordered_isometric_objects_table[i]->sprite));
		}
		...
		update_display_sprite(&(isometric_block_0.sprite));
		update_display_sprite(&(isometric_block_1.sprite));
